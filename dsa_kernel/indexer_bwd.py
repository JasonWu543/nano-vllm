...
def tl_indexer_bwd_impl(
    ...
    block_I: int = 32,  #把topk的分块大小
    num_stages: int = 0,
    num_threads: int = 128,
):
    assert num_stages == 0
    assert topk == tl.math.next_power_of_2(topk)    #要求topk是2的幂
    assert topk % block_I == 0   #要求topk能被block_I整除
    assert heads <= 64 and heads % 8 == 0   #最多64，且为8的倍数
    batch_plus_one = T.symbolic("batch_plus_one")   #offsets长度
    seq_len = T.symbolic("seq_len") #packed后的序列长度
    dtype: str = BF16
    accum_dtype: str = FP32
    index_q_shape = [seq_len, heads, dim]   #定义形状
    ...
    def tl_indexer_bwd_kernel(
            ...
            Offsets: T.Tensor(offsets_shape, INT32),    #前缀和
            TokenIndices: T.Tensor(token_indices_shape, INT32), #每个token对应的序列和token位置
    ):
        with T.Kernel(seq_len, threads=num_threads) as (bx):    #每个bx处理一个token    一个block处理一个token
            i_b, i_t = TokenIndices[bx, 0], TokenIndices[bx, 1]  #i_b表示第几个序列，i_t表示这个序列中的第几个token
            bos = Offsets[i_b]  #该序列的起始位置
            num_blocks = T.ceildiv(topk, block_I)   #计算topk维度要分成多少块
            #分配共享内存
            ...
            #在heads和dim维度对index_q_shared进行缩放
            ...
            for bi_i in T.Pipelined(num_blocks, num_stages=num_stages):
                ...
                T.gemm(
                    ...
                    transpose_A=False,
                    transpose_B=False,
                    clear_accum=True,   #块间不累加，清空再写
                )
                #写回index k，多个query可能命中同一个key，需要原子累加
                for i, j in T.Parallel(block_I, dim):
                    pos = indices_shared[i]
                    if ((pos > -1) & (pos <= i_t)):
