...
def sparse_mla_fwd(
    ...
    indices_shape = [seq_len, kv_group, topk]   #每个token对应topk个位置的索引
    ...
    def main(
            ...
            Indices: T.Tensor(indices_shape, indices_dtype),  # type: ignore    #每个token对应topk个位置的索引
        ...
        with T.Kernel(
            ...
            for i_i in T.Pipelined(NI, num_stages=num_stages):
                #计算的时候通过Indices拿到当前token对应的topk个位置的索引，根据这些索引从KV中拿到对应的KV向量块，进行mask和计算
            ...
            T.copy(sumexp, Lse[bos + s_i, H0:H1])   #写回lse                                                
...
def sparse_mla_fwd_interface(q,
    ...
    _, _, topk = indices.shape  #indices的shape是[seq_len, kv_group, topk]
    ...
    kernel = sparse_mla_fwd(
        ...
        threads=threads)
    out, lse = kernel(q, kv, indices, offsets, token_indices)   #多传一个indices
    return out, lse
