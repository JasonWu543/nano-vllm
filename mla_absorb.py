...
def load_model_first_layer(model: nn.Module, path: str):
    """
    专门为加载 DeepSeek-V2-Lite 第一层设计的加载器
    """
    # 获取模型定义的映射表（如果没有定义，默认为空字典）
    packed_modules_mapping = getattr(model, "packed_modules_mapping", {})
    weight_name_mapping = getattr(model, "weight_name_mapping", {})

    # 过滤关键字：跳过所有 MoE 相关权重
    skip_keywords = [".experts.", ".gate.", ".shared_experts."]

    for file in glob(os.path.join(path, "*.safetensors")):
        with safe_open(file, "pt", "cpu") as f:
            for weight_name in f.keys():
                # --- 1. 层级过滤：只保留 Embedding, Norm, Head 和 第 0 层 ---
                if "layers." in weight_name:
                    try:
                        # 提取层号，例如 "model.layers.1.self_attn..." -> 1
                        layer_idx = int(weight_name.split("layers.")[1].split(".")[0])
                        if layer_idx > 0: # 只要不是第0层，全部跳过
                            continue
                    except:
                        continue
                ...
                try:
                    ...
                    # 某些权重（如后面几层的）在模型里没定义，直接跳过
                    continue
    print("成功加载 Embedding 和第 0 层权重。")
...
# 1. 手动定义默认的 RoPE 初始化逻辑
def llama_rope_init(config, device):
    # DeepSeek-V2 Lite 的 qk_rope_head_dim 是 64
...
# 2. 建立映射表，解决 KeyError: 'default'
ROPE_INIT_FUNCTIONS = {
    "default": llama_rope_init,
}
...
class YoutuMLAttention1(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        #print("kv_b_proj:",self.kv_b_proj.weight.shape)
        #print("k_pass:", k_pass.shape)
        k_pass = k_pass.view(key_shape).transpose(1, 2)
        #print("key_shape:", key_shape)
        #print("qk_nope_head_dim:",self.qk_nope_head_dim)
        #print("v_head_dim",self.v_head_dim)
        k_pass, value_states = torch.split(k_pass, [self.qk_nope_head_dim, self.v_head_dim], dim=-1)
        #print("value_states",value_states.shape,value_states)
        #exit(-1)
...
class YoutuMLAttention(nn.Module):
    """Multi-latent attention with weight absorption optimization.
    
    Instead of explicitly expanding compressed KV through kv_b_proj, this version
    absorbs the kv_b_proj weights into the query side computation:
      - For nope part: q_nope' = q_nope @ W_k_nope^T, then score = q_nope' @ c_kv^T
      - For output: intermediate = attn_weights @ c_kv, then output = intermediate @ W_v^T
    
    This allows the KV cache to store only the compressed representation (kv_lora_rank dim)
    instead of the fully expanded heads, significantly reducing KV cache memory.
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        k_pass_ln = self.kv_a_layernorm(k_pass_raw)

        # ===================== ABSORB (key only) =====================
        # Absorb W_key into q_pass: q_absorbed = q_nope @ W_key
        # This avoids expanding compressed KV through kv_b_proj for key computation
        #
        # kv_b_proj.weight: [H*(nope+v), r]
        # Split into per-head: [H, nope+v, r], take key part: [H, nope, r]
        ...
        W_key = W_kv_b[:, :self.qk_nope_head_dim, :].contiguous()  # [H, nope, r]

        # q_pass absorb via bmm
        q_pass = q_pass.reshape(-1, seq_length, self.qk_nope_head_dim)  # [B*H, S, nope]
        W_key_expanded = W_key.unsqueeze(0).expand(
            batch_size, -1, -1, -1
        ).reshape(-1, self.qk_nope_head_dim, self.kv_lora_rank)  # [B*H, nope, r]
        q_pass = torch.bmm(q_pass, W_key_expanded)  # [B*H, S, r]
        q_pass = q_pass.view(
            batch_size, self.num_heads, seq_length, self.kv_lora_rank
        )  # [B, H, S, r]

        # ===================== Value via kv_b_proj (same path as original) =====================
        # Use the full kv_b_proj to compute value, ensuring bit-exact match
        kv_expanded = self.kv_b_proj(k_pass_ln)  # [B, S, H*(nope+v)]
        kv_expanded = kv_expanded.view(
            batch_size, seq_length, self.num_heads,
            self.qk_nope_head_dim + self.v_head_dim,
        ).transpose(1, 2)  # [B, H, S, nope+v]
        _, value_states = torch.split(
            kv_expanded,
            [self.qk_nope_head_dim, self.v_head_dim],
            dim=-1,
        )  # value_states: [B, H, S, v]

        # ===================== reshape K (compressed) =====================
        k_pass = k_pass_ln.view(
            batch_size, seq_length, 1, self.kv_lora_rank,
        ).transpose(1, 2)  # [B, 1, S, r]
        ...
        # Concatenate absorbed q_pass with q_rot, and compressed k_pass with k_rot
        query_states = torch.cat((q_pass, q_rot), dim=-1)
        key_states = torch.cat((k_pass, k_rot), dim=-1)
        # ===================== cache =====================
        if past_key_values is not None:
...
class YoutuPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        """
        覆盖父类方法，避免由于 transformers 版本更新导致的 RoPE 初始化兼容性问题
        """
        # 注意：这里千万不要写 super()._init_weights(module)
        ...
        if isinstance(module, nn.Linear):
            ...
            module.weight.data.normal_(mean=0.0, std=std) # Lite 版使用标准 std
...
class YoutuModel(YoutuPreTrainedModel):
    ...
    def __init__(self, config: YoutuConfig):
        ...
        self.layers = nn.ModuleList(
            [YoutuDecoderLayer(config, layer_idx) for layer_idx in range(1)]
        )
...
class YoutuForCausalLM(YoutuPreTrainedModel, GenerationMixin):
    ...
    def compute_logits(self, hidden_states: torch.Tensor) -> torch.Tensor:
            """将最后隐藏层状态转换为 logits"""
            # 通常 hidden_states 的形状是 [N, hidden_size]
            # lm_head 将其投影到 [N, vocab_size]
            logits = self.lm_head(hidden_states)
            return logits


__all__ = ["YoutuPreTrainedModel", "YoutuModel", "YoutuForCausalLM_py"]
